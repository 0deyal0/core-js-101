...
const CSS_SELECTOR_TWICE_ERROR = 'Element, id and pseudo-element should not occur more then one time inside the selector';
const CSS_SELECTOR_NOT_SORTED_ERROR = 'Selector parts should be arranged in the following order: element, id, class, attribute, pseudo-class, pseudo-element';

const cssSelectorBuilder = {
  element(value) {
    if (this.res && this.res.filter((val) => val.type === 'element').length !== 0) {
      throw new Error(CSS_SELECTOR_TWICE_ERROR);
    }
