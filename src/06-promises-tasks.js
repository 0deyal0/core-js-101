...
function chainPromises(array, action) {
  return array.reduce((accumulator, value) => accumulator.then(
    (results) => Promise.resolve(value).then((result) => [...results, result]).catch(() => { }),
  ),
  Promise.resolve([]))
    .then((arr) => arr.reduce((prev, cur) => action(prev, cur)))
    .catch(() => { });
}
